...
//	 
		...
		$(document).on("click", ".item-add", function (e) {
			...
			$.get("calendar3.html", $frmBooking.find("input[name='id'], input[name='tmp_hash'], #date_from, #hour_from, #minute_from, #date_to, #hour_to, #minute_to").serialize()).done(function (data) {
			...
			if ($this.hasClass("asSlotSelected")) {
				...
				$.ajax({
			        url: '/appointment/saveCalendar', 
			        type: "POST",
			        contentType: "application/json; charset=utf-8",
			        dataType: "json", 
			        success: function (result) {
			        	window.location.href = "/appointment/calendar/";   
			        }});
